...
Plugin.PermamuteFile = "config://PermamutedPlayers.json" --What's the name of the file?
...
function Plugin:Initialise()
    self.PermamutedPlayers = Shine.LoadJSONFile(self.PermamuteFile) or {}
    self:CreateCommands()
    self.Enabled = true
    return true
end
...
function Plugin:IsClientPermamuted(Client, ChannelType)
    ...
    self:CheckRemoveClientPermamute(Client)
    return false
end

function Plugin:CheckRemoveClientPermamute(Client)
    local ClientID = "" .. Client:GetUserId() or 0
    local PermamuteData = self.PermamutedPlayers[ClientID]
    local canBeRemoved = true

    for channel, value in PermamuteData do
    ...
    if canBeRemoved then
        ...
        Shine.SaveJSONFile(self.PermamutedPlayers, self.PermamuteFile)
    end
end
...
function Plugin:CreateCommands()
    local function PermamutePlayer(Client, Target, ChannelType, Duration)
        ...
        if ChannelType ~= "voice" and ChannelType ~= "chat" and ChannelType ~= "text" and ChannelType ~= "both" and ChannelType ~= "all"  then
            Shared.Message("<Target> <voice|chat|all> [duration]")
            Shared.Message("Failed to specify type of permamute (voice or chat).")
            return
        end
